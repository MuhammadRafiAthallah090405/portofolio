...
function renderProjects(data) {
    ...
    data.forEach(project => {
        ...
        `;
        const imageFrame = projectCard.querySelector('.project-image');
        imageFrame.dataset.title = project.title;
        const projectImage = projectCard.querySelector('img');
        projectImage.addEventListener('error', () => {
            projectImage.remove();
            imageFrame.setAttribute('aria-label', `${project.title} image unavailable`);
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setActiveNav() {
        const viewportCheckPoint = window.scrollY + (window.innerHeight * 0.45);
        ...
        sections.forEach(section => {
            ...
            if (viewportCheckPoint >= sectionTop && viewportCheckPoint < sectionBottom) {
